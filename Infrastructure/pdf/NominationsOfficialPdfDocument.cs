...
public class NominationsOfficialPdfDocument : IDocument
{
    private readonly NominationsReportData _data;
    private readonly List<Nomination> _nominations;
    private readonly byte[]? _logoBytes;

    public NominationsOfficialPdfDocument(NominationsReportData data, List<Nomination> nominations, byte[]? logoBytes)
    {
        _data = data;
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            page.Content().PaddingTop(12).Column(col =>
            {
                ...
                col.Item().PaddingTop(6).Text("Liste des nominations (extrait)").SemiBold().FontSize(12);
                col.Item().Element(ComposeNominationsTable);
            });
    ...
    void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text("Rapport Officiel — Nominations").FontSize(12);
    ...
    void ComposeSummaryCards(IContainer container)
    {
        container.Row(row =>
        {
            ...
            row.RelativeItem().Element(c => StatCard(c, "Total nominations", _data.TotalNominations.ToString()));
            row.RelativeItem().Element(c => StatCard(c, "Validées", _data.NominationsValidees.ToString()));
            row.RelativeItem().Element(c => StatCard(c, "En attente", _data.NominationsEnAttente.ToString()));
            row.RelativeItem().Element(c => StatCard(c, "Rejetées", _data.NominationsRejetees.ToString()));
        });
    }
    ...
    void ComposeStatsTables(IContainer container)
    {
        container.Row(row =>
        {
            ...
            row.RelativeItem().Element(c => KeyValueTable(c, "Top postes (10)", _data.ByPoste
                .OrderByDescending(x => x.Value).Take(10).ToDictionary(x => x.Key, x => x.Value)));
        });
    }
    ...
    void ComposeNominationsTable(IContainer container)
    {
        container.Border(1).BorderColor(Colors.Grey.Lighten2).Padding(10).Table(t =>
        {
            t.ColumnsDefinition(cols =>
            {
                ...
                cols.RelativeColumn(2); // date début
                cols.RelativeColumn(2); // date fin
            ...
            t.Header(h =>
            {
                ...
                h.Cell().Text("Poste").SemiBold();
                h.Cell().Text("Date Début").SemiBold();
                h.Cell().Text("Date Fin").SemiBold();
                h.Cell().Text("Statut").SemiBold();
            });

            foreach (var n in _nominations.Take(200)) // extrait
            {
                t.Cell().Text($"{n.Scout?.Nom} {n.Scout?.Prenoms}");
                t.Cell().Text(n.Groupe?.Nom ?? "");
                t.Cell().Text(n.Poste);
                t.Cell().Text(n.DateDebut.ToString("dd/MM/yyyy"));
                t.Cell().Text(n.DateFin?.ToString("dd/MM/yyyy") ?? "-");
                t.Cell().Text(n.Statut.ToString());
            }
        });
