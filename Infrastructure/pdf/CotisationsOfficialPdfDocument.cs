...
public class CotisationsOfficialPdfDocument : IDocument
{
    private readonly CotisationsReportData _data;
    private readonly List<Cotisation> _cotisations;
    private readonly byte[]? _logoBytes;

    public CotisationsOfficialPdfDocument(CotisationsReportData data, List<Cotisation> cotisations, byte[]? logoBytes)
    {
        _data = data;
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            page.Content().PaddingTop(12).Column(col =>
            {
                ...
                col.Item().PaddingTop(6).Text("Liste des cotisations (extrait)").SemiBold().FontSize(12);
                col.Item().Element(ComposeCotisationsTable);
            });
    ...
    void ComposeHeader(IContainer container)
    {
        container.Row(row =>
        {
            ...
            row.RelativeItem().Column(col =>
            {
                ...
                col.Item().Text("Rapport Officiel — Cotisations").FontSize(12);
    ...
    void ComposeSummaryCards(IContainer container)
    {
        container.Row(row =>
        {
            ...
            row.RelativeItem().Element(c => StatCard(c, "Total cotisations", _data.TotalCotisations.ToString()));
            row.RelativeItem().Element(c => StatCard(c, "Montant attendu", _data.MontantTotalAttendu.ToString("N2") + " FCFA"));
            row.RelativeItem().Element(c => StatCard(c, "Montant payé", _data.MontantTotalPaye.ToString("N2") + " FCFA"));
            row.RelativeItem().Element(c => StatCard(c, "Taux de cotisation", _data.TauxCotisation.ToString("N1") + "%"));
        });
    }
    ...
    void ComposeCotisationsTable(IContainer container)
    {
        container.Border(1).BorderColor(Colors.Grey.Lighten2).Padding(10).Table(t =>
        {
            t.ColumnsDefinition(cols =>
            {
                cols.RelativeColumn(3); // scout
                cols.RelativeColumn(3); // groupe
                cols.RelativeColumn(2); // période
                cols.RelativeColumn(2); // montant attendu
                cols.RelativeColumn(2); // montant payé
                cols.RelativeColumn(2); // statut
            });

            t.Header(h =>
            {
                h.Cell().Text("Scout").SemiBold();
                h.Cell().Text("Groupe").SemiBold();
                h.Cell().Text("Période").SemiBold();
                h.Cell().Text("Attendu").SemiBold();
                h.Cell().Text("Payé").SemiBold();
                h.Cell().Text("Statut").SemiBold();
            });

            foreach (var c in _cotisations.Take(200)) // extrait
            {
                t.Cell().Text($"{c.Scout?.Nom} {c.Scout?.Prenoms}");
                t.Cell().Text(c.Groupe?.Nom ?? "");
                t.Cell().Text(c.Periode);
                t.Cell().Text(c.MontantAttendu.ToString("N2"));
                t.Cell().Text(c.MontantPaye.ToString("N2"));
                t.Cell().Text(c.Statut.ToString());
            }
        });
